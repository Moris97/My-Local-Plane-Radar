...
- **Major airports shown on the offline basemap** (effort: small, impact:
  ...
  2026-08-02.
- ~~**Circling-aircraft notification**~~ **Done, 2026-08-08 (v2.2.1, radius
  widened for military orbits in v2.2.2, category/military allowlist added
  in v2.2.3).** Requested 2026-07-28, built exactly to spec:
  `server/src/notifications/circling-detector.js`'s
  `recordAndCheckCircling` tracks a signed cumulative heading change per
  hex over a rolling window, true once it passes 360° while the window's
  own centroid stays within a wide-enough radius to also catch a slow
  AWACS/tanker-scale orbit, not just a tight one. `isCirclingRelevant`
  gates the whole thing on aircraft category/military status (large
  civilian, any helicopter, or any military aircraft — light aircraft and
  gliders excluded, per real-world measurement that light-aircraft circuit
  training was the dominant false-positive, more so than gliders alone).
  New `circlingEnabled` setting (default on), wired into the on-map
  toast/glow feature and smart-home like every other standing-condition
  rule — see CLAUDE.md's Notification engine section for the full design.
